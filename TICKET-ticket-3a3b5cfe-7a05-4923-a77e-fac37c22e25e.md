リポジトリ構成と既存のICE/E2E実装を確認し、必要なRFCも調べて要件を具体化します。

  │ "ICE-TCP|ice tcp|TCP candidate|tcpType|tcptype|active|passive|so"
  │ (packages/ice, packages/webrtc, e2e)

  │ "packages/{ice,webrtc}/**/*"

  │ "**/AGENTS.md"

  │ "candidate|host candidate|srflx|relay|component|foundation|priority" in *.ts
  │ (packages/ice, packages/webrtc)
  ...
  │ packages/ice/src/types/model.ts

  │ packages/ice/src/stun/protocol.ts

  │ packages/ice/src/turn/protocol.ts

  │ packages/webrtc/src/sdp.ts

  │ packages/webrtc/tests/transport/ice.test.ts
  ...
  │ "iceLite|trickle|restart|datachannel|offer|answer|peerConnection" in *.ts
  │ (e2e/server, e2e/tests)

  │ "e2e/tests/**/*.ts"

既存のE2Eパターンとサーバ側ハンドラを確認して、ICE-TCP用に必要なテスト面も詰めます。

  │ e2e/tests/datachannel/turnRelay.test.ts

  │ e2e/tests/fixture.ts

  │ e2e/tests/datachannel/iceLite.test.ts

  │ e2e/tests/ice/trickle.test.ts

  │ e2e/server/main.ts

  │ e2e/server/handler/datachannel/turnRelay.ts

  │ e2e/server/handler/datachannel/iceLite.ts

  │ e2e/server/fixture.ts

  │ e2e/server/turn.ts

  │ "remoteCandidate\.transport|transport === "udp"|tcptype" in *.ts
  │ (packages/ice/src, packages/webrtc/src)

  │ "candidateToSdp|candidateFromSdp" in *.ts (packages/webrtc/src)

  │ "class TcpTransport|class TlsTransport|export class TcpTransport|export
  │ class TlsTransport" in *.ts (packages)

  │ packages/common/src/transport.ts

  │ packages/webrtc/src/secureTransportManager.ts

  │ packages/webrtc/src/types/domain.ts

  │ packages/webrtc/src/sdp.ts

  │ "iceServers|turnTransport|iceLite|iceUseIpv6|iceFilterCandidatePair|iceInter
  │ faceAddresses" in *.ts (packages/webrtc/src)
...
● Fetching web content https://www.rfc-editor.org/rfc/rfc8445 · url: "https://www.rfc-editor.org/rf…

● Fetching web content https://www.rfc-editor.org/rfc/rfc4571 · url: "https://www.rfc-editor.org/rf…

● Fetching web content https://www.rfc-editor.org/rfc/rfc4145 · url: "https://www.rfc-editor.org/rf…

● Fetching web content https://www.rfc-editor.org/rfc/rfc6544 · url: "https://www.rfc-editor.org/rf…

● Fetching web content https://www.rfc-editor.org/rfc/rfc8839 · url: "https://www.rfc-editor.org/rf…

● Web Search (MCP: github-mcp-server) · What RFC requirements are most relevant for implementing IC…

  │ "tcptype|tcpType|TCP" in *.ts (e2e, packages/ice, packages/webrtc)

● Web Search (MCP: github-mcp-server) · RFC 6544 active passive so candidate pairing rules and whet…

● Web Search (MCP: github-mcp-server) · Does Chromium WebRTC currently support ICE-TCP candidates a…

● Fetching web content https://www.rfc-editor.org/rfc/rfc8835 · url: "https://www.rfc-editor.org/rf…

● Fetching web content https://w3c.github.io/webrtc-ice/ · url: "https://w3c.github.io/webrtc-ice/"…

✗ Fetching web content https://webrtc.googlesource.com/src/+/HEAD/p2p/g3doc/ice.md?format=TEXT · ur…
    https://webrtc.googlesource.com/src/+/HEAD/p2p/g3doc/ice.md?format=TEXT -
    status code 400

  │ "pairRemoteCandidate|pairLocalProtocol|checkIncoming\(|buildRequest\(|USE-CA
  │ NDIDATE|PRIORITY" (packages/ice/src/ice.ts)
...
● Fetching web content https://www.rfc-editor.org/rfc/rfc8835 · url: "https://www.rfc-editor.org/rf…
  ...
  │ "StunProtocol\(|new
  │ Candidate\(|candidatePriority\("relay"\)|candidateFoundation\("host", "udp""
  │ (packages/ice/src/ice.ts)

  │ "turnTransport|forceTurnTCP|parseIceServerUrl|resolveParsedTurnTransport"
  │ (packages/webrtc/src/utils.ts, packages/webrtc/src/peerConnection.ts)

  │ packages/ice/src/ice.ts


  │ "chrome|browser|run-ci|E2E_PORT|install:browsers" in *.{js,ts,mjs} (e2e)

  │ packages/ice/src/turn/protocol.ts

  │ "createStunOverTurnClient|transport: "tcp"|transport:
  │ "tls"|TcpTransport\.init|TlsTransport\.init"
  │ (packages/ice/src/turn/protocol.ts)
  ...
  │ "onData\.subscribe|send\(|sendData\(|transport\.send|dtls" in *.ts
  │ (packages/webrtc/src/transport, packages/dtls/src)

  │ packages/webrtc/src/transport/dtls.ts

  │ "function createIceTransport|const createIceTransport|createIceTransport\("
  │ (packages/webrtc/src/transport/dtls.ts)

  │ packages/webrtc/src/transport/dtls.ts

  │ "class IceTransport" (packages/webrtc/src/transport/dtls.ts)
