...
* Docs-only edits: no code validation required. Public API / Typedoc surface changes: run `npm run doc:check` so committed `doc/` and `packages/*/doc` match sources (also part of `npm run ci`).
