...
describe("rtc/transport/srtp", () => {
  test("test_rtp", async () => {
    ...
    session1.sendRtp(RTP);
    const rtp = await session2.onSrtp.asPromise();
    expect(rtp.serialize()).toEqual(RTP);

    const packets = RtcpPacketConverter.deSerialize(RTCP);
    session1.sendRtcp(packets);
    const rtcp = await session2.onSrtcp.asPromise();
...
> {
  ...
  const session1 = new RTCDtlsTransport(transport1, [certificate1], [1]);

  const certificate2 = RTCCertificate.unsafe_useDefaultCertificate();
  const session2 = new RTCDtlsTransport(transport2, [certificate2], [1]);
