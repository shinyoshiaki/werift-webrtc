 ...
 */
export async function onClientHello(
  this: Dtls13Host,
  body: Buffer,
  messageSeq: number,
): Promise<void> {
  ...
  const sigExt = ch.extensions.find((e) => e.type === SignatureAlgorithms.type);
  if (!sigExt) {
    throw new DtlsProtocolError(
  ...
  if (this.addressValidation === "dtls-cookie" && !this.addressValidated) {
    ...
    this.transcript.replaceWithPrecomputedMessageHash(verified.ch1MessageHash);
    ...
    if (this.firstClientHelloBody) {
      ...
      if (verified.selectedGroup !== undefined) {
        ...
        if (shares.length !== 1 || shares[0].group !== verified.selectedGroup) {
          throw new DtlsProtocolError(
...
export async function sendServerFlight(this: Dtls13Host): Promise<void> {
