...
function buildChWithBadLegacyVersion(): Buffer {
  const kp = generateKeyPair(NamedCurveAlgorithm.x25519_29);
  const curves = EllipticCurves.createEmpty();
  curves.data = [NamedCurveAlgorithm.x25519_29] as any;
  const ch = new ClientHello(
    { major: 254, minor: 255 }, // not 0xfefd
    new DtlsRandom(),
    Buffer.alloc(0),
    Buffer.alloc(0),
    [CipherSuite.TLS_AES_128_GCM_SHA256_0x1301],
...
function buildChWithBadCompression(): Buffer {
  const kp = generateKeyPair(NamedCurveAlgorithm.x25519_29);
  const curves = EllipticCurves.createEmpty();
  curves.data = [NamedCurveAlgorithm.x25519_29] as any;
  const ch = new ClientHello(
...
describe("e2e/self13 ClientHello validation", () => {
  ...
  test("rejects ClientHello with wrong legacy_version", async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      server.onError.subscribe((e) => {
        clearTimeout(timer);
        try {
          expect(e.message).toMatch(/illegal_parameter|legacy_version|0xfefd/i);
          server.close();
          resolve();
        } catch (err) {
          reject(err);
      ...
      serverTransport.onData?.(buildChWithBadLegacyVersion(), [
        "127.0.0.1",
        9,
      ] as any);
    });
  ...
  test("rejects ClientHello with non-[0] compression_methods", async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      serverTransport.onData?.(buildChWithBadCompression(), [
        "127.0.0.1",
        9,
      ] as any);
    });
