...
Opt-in: set `WERIFT_PION_SPED` to a wrapper that supports `verify`, `-empty-ack`, and decode via `DtlsInStunAttribute.GetFrom` / `DtlsInStunAckAttribute.GetFrom` (`sped-getfrom`). Unset skips so default `npm test` stays green. `npm run test:pion-sped` (sets `WERIFT_PION_SPED_REQUIRED=1`) **fails** unless `WERIFT_PION_SPED` or `WERIFT_PION_SPED_AUTO_BUILD=1` is set (no skip / local fallback). A missing or incompatible `WERIFT_PION_SPED` also fails that script. `WERIFT_PION_SPED_AUTO_BUILD=1` builds `./pion-sped` with `go` when the env path is unset.
...
`decode` also runs released pion/ice `sped.go` `DtlsInStunAttribute.GetFrom` and `DtlsInStunAckAttribute.GetFrom` (parsed DATA value / ACK CRCs). A GetFrom error fails the command.
