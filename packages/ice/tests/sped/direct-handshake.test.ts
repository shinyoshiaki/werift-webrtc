...
describe("SPED hybrid direct-handshake readiness", () => {
  ...
  it("別pairの成功応答でも認証済みpinned pathを維持してhandshakeを再送できる", async () => {
    // Arrange: incoming SPED の認証済み経路と outgoing check の経路が異なる。
    const { connection, pair } = arrangeFullCheck();
    const handle = attachTestSped(connection);
    const pinned = spedPair(new SpedProtocolMock(), "host");
    pinned.requestsReceived = 1;
    handle.runtime.lastPath = pinned;
    pair.responsesReceived = 1;

    try {
      // Act: 別pairの応答を契機に、固定済みの認証済み経路だけを direct にする。
      const ready = handle.runtime.tryMarkDirectHandshakeReady(
        pair,
        connection.generation,
      );

      // Assert: 経路を移さず handshake の再送を許可し、application は consent 前で遮断する。
      expect(ready).toBe(true);
      expect(handle.runtime.lastPath).toBe(pinned);
      expect(handle.runtime.isHandshakeDirectReady()).toBe(true);
