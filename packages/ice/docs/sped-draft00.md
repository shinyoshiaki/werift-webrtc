...
- Peer SPED support is decided on the first **current-generation authenticated** Binding on a **confirmed eligible** pair (UDP host/srflx, TCP host/prflx): DATA present (including empty) → supported; missing → `fallback`. UDP prflx is unconfirmed (may later be trickle relay): missing DATA does not lock unsupported; non-empty DATA pins the association (incoming inject **and** outgoing L1) until signaling upgrades the candidate to host/srflx or relay. After `lastPath` is pinned, other eligible pairs still get empty C070 (no L1, no ACK, no inject) so reorder cannot look like a non-SPED peer.
