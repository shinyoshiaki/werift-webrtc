...
export const GCC_KNOWN_DIFFERENCES = [
  ...
  "AIMD: TimeToReduceFurther (RTT spacing + throughput check) and hold-after-decrease ported; RTT proxy = feedback arrival − last send (not full ICE/STUN CorrectedRtt which adds time-since-last-send when feedback stalls); AIMD decrease spacing clamps RTT to [10, 2000] ms while IsRttAboveLimit uses the unclamped proxy vs 3s and applies RttBasedBackoff drop (×0.8 / 1s, floor max(minBitrate, 5kbps)); probe accept uses setEstimate (preserves RTT / max-bitrate stats), not full reset",
