 ...
 * window after the first {@link GccBandwidthEstimator.updatePacketsLost}
 ...
 */
export const kStartPhaseMs = 2_000;

/**
 * pin `UpdatePacketsLost` `kLimitNumPackets` = 20.
 * Fraction-loss is not published until this many packets accumulate.
 */
export const kStartPhaseLossReportMinPackets = 20;
...
/**
 * pin LossBasedBweV2 `UseInStartPhase` FieldTrial default (`true`).
 * When ready, LossBased replaces the 2s delay-based startup ramp.
 */
export const kLossBasedUseInStartPhase = true;
...
/** SendingRateSmoothingFactor default 0.0 — no smoothing. */
export const kLossBasedSendingRateSmoothingFactor = 0;
/**
 * BwBackoffLowerBoundFactor default 1.0.
 * Applied to the acknowledged-rate candidate in GetCandidates.
 */
export const kLossBasedBandwidthBackoffLowerBoundFactor = 1;
...
export const kLossBasedNotUseAckedRateInAlr = true;
/** MedianSendingRateFactor default 2.0 (byte-loss spike filter). */
export const kLossBasedMedianSendingRateFactor = 2;
...
/**
 * pin `kDefaultPaceMultiplier` — used until the first transport feedback
 * (`first_transport_feedback_received_ == false`).
 */
export const kDefaultPaceMultiplier = 2.5;
/**
 * pin `kDefaultPaceMultiplierWithSendSideBwe` — after first TWCC
 * (send-side BWE is active).
 */
export const kDefaultPaceMultiplierWithSendSideBwe = 1.1;

/**
 * pin ProbeControllerConfig `min_probe_delta` default (2ms).
 * Inter-burst spacing after the initial exponential session.
 */
export const kMinProbeDeltaMs = 2;
/**
 * pin ProbeControllerConfig `initial_min_probe_delta` default (20ms).
 * Used for the first 3x/6x exponential clusters.
 */
export const kInitialMinProbeDeltaMs = 20;
/**
 * pin BitrateProberConfig `max_probe_delay` default (10ms).
 * An active cluster whose next_probe_time is this late is discarded.
 */
export const kMaxProbeDelayMs = 10;
/**
 * pin BitrateProber `kMaxPendingProbeClusters`.
 */
export const kMaxPendingProbeClusters = 5;
...
/**
 * pin BitrateProber `kProbeClusterTimeout` (5s).
 * Drops **queued** (not yet activated) clusters whose `requested_at` is
 * older than this. Not an active-packet scheduling budget — late active
 * clusters are discarded via {@link kMaxProbeDelayMs} instead.
 */
...
/**
 * pin LossBasedBweV2 `PaddingDuration` FieldTrial default (`Seconds(2)`).
 * Non-zero enables `increase_using_padding` instead of `increasing`.
 */
export const kLossBasedPaddingDurationMs = 2_000;
/**
