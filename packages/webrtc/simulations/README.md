...
- RTT: **min_propagation_rtt** = min(feedback_rtt − (max_recv − recv)) を `UpdatePropagationRtt` へ；IsRttAboveLimit は CorrectedRtt（raw max_feedback_rtt ではない）。>3s は probe 禁止 **および** pin 相当の target ×0.8 drop（1s 間隔・5kbps floor）。初回 `rtpPacketSent` は pin OnSentPacket（`UpdatePropagationRtt(send, 0)` seed のみ）。`UpdateEstimate` / `ProbeController::Process` は 25ms `process()`（UpdateEstimate → SetAlrStartTime → Process → MaybeTrigger）
