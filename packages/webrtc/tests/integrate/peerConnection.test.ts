...
import type { RtpRouter } from "../../src/media/router";
...
describe("peerConnection", () => {
  ...
  test("post-connect fingerprint変更のremote offerはcurrent DTLS associationを維持する", async () => {
    ...
    try {
      ...
      const applicationDtls = caller.dtlsTransports[0]!;
      ...
      expect(channel.readyState).toBe("closed");
      expect(caller.sctp).toBeNull();
      expect(applicationDtls.state).toBe("closed");
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
    }
  }, 30_000);
  ...
  test("offerer交代のanswerは既存DTLS associationをreuseする", async () => {
    // Arrange: A offer / B answer で DataChannel を接続する。
    const pcA = new RTCPeerConnection({ iceServers: [] });
    const pcB = new RTCPeerConnection({ iceServers: [] });

    try {
      const [dcA, dcB] = await createDataChannelPair({}, pcA, pcB);
      const dtlsB = pcB.dtlsTransports[0]!;
      expect(dtlsB.state).toBe("connected");

      // Act: Bがsubsequent offererになり、Aがsetup:passiveのanswerを返す。
      await pcB.setLocalDescription(await pcB.createOffer());
      await pcA.setRemoteDescription(pcB.localDescription!);
      const answer = await pcA.createAnswer();
      expect(answer.sdp).toMatch(/a=setup:passive/);
      await pcA.setLocalDescription(answer);
      await pcB.setRemoteDescription(pcA.localDescription!);

      // Assert: 新association要求ではなく、同一DTLS objectとDataChannelが続く。
      expect(pcB.dtlsTransports[0]).toBe(dtlsB);
      expect(dtlsB.state).toBe("connected");
      expect(pcB.connectionState).toBe("connected");
      expect(dcA.readyState).toBe("open");
      const received = dcB.onMessage.asPromise(5_000);
      dcA.send("offerer-switch");
      expect((await received)[0].toString()).toBe("offerer-switch");
    } finally {
      await Promise.allSettled([pcA.close(), pcB.close()]);
    }
  }, 30_000);

  test("remote answerのnegotiated setup role変更はSRDでNotSupportedErrorになる", async () => {
    // Arrange: offerer=DTLS server の接続済みDataChannelを用意する。
    const caller = new RTCPeerConnection({ iceServers: [] });
    const callee = new RTCPeerConnection({ iceServers: [] });
    const channel = caller.createDataChannel("role-flip");
    ...
    try {
      ...
      expect(dtls.role).toBe("server");

      // Act: subsequent offerに対し、remote answerだけ setup:passive へ変える。
      await caller.setLocalDescription(await caller.createOffer());
      await callee.setRemoteDescription(caller.localDescription!);
      const answer = await callee.createAnswer();
      expect(answer.sdp).toMatch(/a=setup:active/);
      const flipped = answer.sdp.replace(/a=setup:active/g, "a=setup:passive");
      await expect(
        caller.setRemoteDescription({ type: "answer", sdp: flipped }),
      ).rejects.toThrow("DTLS association replacement is not implemented");

      // Assert: live DTLSとDataChannelはSRD失敗後もconnectedのまま。
  ...
  test("SCTP BUNDLE migrationはrebind後のstopTransport失敗でもcommitする", async () => {
    // Arrange: unbundled DataChannelをBUNDLE renegotiationのanswer直前まで進める。
    const { caller, callee, channel, remoteChannel } =
      await createConnectedUnbundledPair();
    ...
    try {
      ...
      const secureManager = (
        callee as unknown as {
          secureManager: {
            stopTransport: (transport: unknown) => Promise<void>;
          };
        }
      ).secureManager;
      const originalStop = secureManager.stopTransport.bind(secureManager);
      const originalSetDtls = callee.sctp!.setDtlsTransport.bind(callee.sctp);
      let rebound = false;
      callee.sctp!.setDtlsTransport = (transport) => {
        originalSetDtls(transport);
        rebound = true;
      };
      secureManager.stopTransport = async (transport) => {
        if (rebound) {
          throw new Error("injected stopTransport failure");
        }
        return originalStop(transport);
      };

      try {
        // Act: SCTP rebind後のtransport stopだけ失敗させる。
        await callee.setLocalDescription(await callee.createAnswer());
      } finally {
        secureManager.stopTransport = originalStop;
        callee.sctp!.setDtlsTransport = originalSetDtls;
      }
      ...
      // Assert: 不可逆commit後のstop失敗はrollbackせずDataChannelを維持する。
      expect(rebound).toBe(true);
      expect(callee.signalingState).toBe("stable");
      const receivedMessage = remoteChannel.onMessage.asPromise(5_000);
      channel.send("sctp-postcommit");
      expect((await receivedMessage)[0].toString()).toBe("sctp-postcommit");
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
    }
  }, 30_000);
  ...
  test("a=sctp-port:0の後createDataChannelは新しいRTCSctpTransportで再確立する", async () => {
    // Arrange: SCTPをport 0で閉じたあと、アプリが新しいDataChannelを作る。
    const { caller, callee, channel } = await createConnectedUnbundledPair();
    const oldCallerSctp = caller.sctp!;
    const oldCalleeSctp = callee.sctp!;
    const applicationDtls = oldCallerSctp.dtlsTransport;
    ...
    try {
      ...
      // Act: 新しいDataChannel作成がnegotiationneededを起こし、O/Aで再確立する。
      const negotiationNeeded = new Promise<void>((resolve) => {
        caller.onnegotiationneeded = () => resolve();
      });
      const remoteChannelPromise = new Promise<RTCDataChannel>((resolve) => {
        callee.ondatachannel = ({ channel: remoteChannel }) =>
          resolve(remoteChannel);
      });
      const dc2 = caller.createDataChannel("reopen");
      await negotiationNeeded;
      expect(caller.sctp).not.toBeNull();
      expect(caller.sctp).not.toBe(oldCallerSctp);
      expect(caller.sctp!.dtlsTransport).toBe(applicationDtls);

      await setLocalOfferWithSctpPort(caller, caller.sctp!.port);
      await callee.setRemoteDescription(caller.localDescription!);
      await callee.setLocalDescription(await callee.createAnswer());
      await caller.setRemoteDescription(callee.localDescription!);

      // Assert: 新しいSCTP objectで開き、既存DTLS上でメッセージが通る。
      expect(callee.sctp).not.toBeNull();
      expect(callee.sctp).not.toBe(oldCalleeSctp);
      expect(caller.sctp!.dtlsTransport).toBe(applicationDtls);
      ...
      dc2.send("reopened");
      expect((await received)[0].toString()).toBe("reopened");
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
    }
  }, 30_000);
  ...
  test("unbundled RTP sessionは同じextmap IDを別URIへ使える", async () => {
    // Arrange: audio=MID, video=TWCC を別RTP sessionとして交渉する。
    const headerExtensions = {
      audio: [useSdesMid()],
    ...
    try {
      ...
      // Assert: 別sessionなら extmap:1 を異なるURIへ割り当ててもSRDは成功する。
      expect(caller.localDescription!.sdp).toMatch(
        new RegExp(`m=audio[\\s\\S]*a=extmap:1 ${RTP_EXTENSION_URI.sdesMid}`),
      );
      expect(caller.localDescription!.sdp).toMatch(
        new RegExp(
          `m=video[\\s\\S]*a=extmap:1 ${RTP_EXTENSION_URI.transportWideCC}`,
        ),
      );
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
  ...
  test("disableで確立した別sessionのextmap IDはmax-compatのBUNDLE再offerでremapしない", async () => {
    // Arrange: A=disable が audio MID=1 / video TWCC=1 を別sessionでofferする。
    const headerExtensions = {
      audio: [useSdesMid()],
      video: [useTransportWideCC()],
    };
    ...
    try {
      ...
      await callee.setLocalDescription(await callee.createAnswer());
      await caller.setRemoteDescription(callee.localDescription!);
      expect(extmapsInMedia(caller.localDescription!.sdp, "audio")[0]).toEqual(
        expect.objectContaining({ id: 1, uri: RTP_EXTENSION_URI.sdesMid }),
      );
      expect(extmapsInMedia(caller.localDescription!.sdp, "video")[0]).toEqual(
  ...
  test("BUNDLE offerはaudioとvideoで異なる先頭extmapに別IDを割り当てる", async () => {
    ...
    try {
      await caller.setLocalDescription(await caller.createOffer());
      const audioExt = extmapsInMedia(caller.localDescription!.sdp, "audio");
      const videoExt = extmapsInMedia(caller.localDescription!.sdp, "video");

      // Act: 生成したBUNDLE offerをwerift同士でSRDする。
      await callee.setRemoteDescription(caller.localDescription!);
      await callee.setLocalDescription(await callee.createAnswer());
      await caller.setRemoteDescription(callee.localDescription!);

      // Assert: 先頭extensionが違えばIDも違い、自己相互接続できる。
      expect(audioExt[0]!.uri).toBe(RTP_EXTENSION_URI.audioLevelIndication);
      expect(videoExt[0]!.uri).toBe(RTP_EXTENSION_URI.sdesMid);
      expect(audioExt[0]!.id).not.toBe(videoExt[0]!.id);
      expect(callee.signalingState).toBe("stable");
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
    }
  ...
  test("BUNDLE offerはaudio/videoで並びが違っても同じURIに同じIDを使う", async () => {
    // Arrange: 同じ2 URIをaudioとvideoで逆順に載せる。
    const headerExtensions = {
      audio: [useSdesMid(), useTransportWideCC()],
      video: [useTransportWideCC(), useSdesMid()],
    };
    const caller = new RTCPeerConnection({ iceServers: [], headerExtensions });
    const callee = new RTCPeerConnection({ iceServers: [], headerExtensions });
    caller.addTransceiver("audio", { direction: "sendonly" });
    ...
    try {
      ...
      // Act: 並びが違うBUNDLE offerをwerift同士でSRDする。
      await callee.setRemoteDescription(caller.localDescription!);
      await callee.setLocalDescription(await callee.createAnswer());
      await caller.setRemoteDescription(callee.localDescription!);

      // Assert: URIごとのIDは一致し、自己相互接続できる。
      expect(
        audioExt.find((ext) => ext.uri === RTP_EXTENSION_URI.sdesMid)?.id,
      ).toBe(videoExt.find((ext) => ext.uri === RTP_EXTENSION_URI.sdesMid)?.id);
      expect(
        audioExt.find((ext) => ext.uri === RTP_EXTENSION_URI.transportWideCC)
          ?.id,
      ).toBe(
        videoExt.find((ext) => ext.uri === RTP_EXTENSION_URI.transportWideCC)
          ?.id,
  ...
  test("確立済みBUNDLEへ追加するm-lineは既存extmap IDを維持する", async () => {
    // Arrange: audio-levelだけをBUNDLEで確立してからvideo MIDを足す。
    const headerExtensions = {
      audio: [useAudioLevelIndication()],
      video: [useSdesMid()],
    };
    ...
    try {
      ...
      await callee.setLocalDescription(await callee.createAnswer());
      await caller.setRemoteDescription(callee.localDescription!);
      const audioId = extmapsInMedia(caller.localDescription!.sdp, "audio")[0]!
        .id;

      // Act: 確立済みBUNDLEへvideo m-lineを追加する。
      caller.addTransceiver("video", { direction: "sendonly" });
      callee.addTransceiver("video", { direction: "recvonly" });
      await caller.setLocalDescription(await caller.createOffer());
      const nextAudio = extmapsInMedia(caller.localDescription!.sdp, "audio");
      const nextVideo = extmapsInMedia(caller.localDescription!.sdp, "video");
      await callee.setRemoteDescription(caller.localDescription!);
      await callee.setLocalDescription(await callee.createAnswer());
      await caller.setRemoteDescription(callee.localDescription!);

      // Assert: 既存audio mappingは維持し、新しいURIは別IDになる。
      expect(nextAudio[0]!.id).toBe(audioId);
      expect(nextVideo[0]!.uri).toBe(RTP_EXTENSION_URI.sdesMid);
      expect(nextVideo[0]!.id).not.toBe(audioId);
      expect(callee.signalingState).toBe("stable");
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
    }
  ...
  test("BUNDLE joinの未harmonize extmapはSRDでrejectしICEを壊さない", async () => {
    // Arrange: unbundled で audio MID と video TWCC を別sessionに置く。
    const headerExtensions = {
      audio: [useSdesMid()],
      video: [useTransportWideCC()],
    };
    ...
    try {
      ...
      await caller.setLocalDescription({
        type: "offer",
        sdp: stripBundleGroup(initialOffer.sdp),
      });
      await callee.setRemoteDescription(caller.localDescription!);
      await callee.setLocalDescription(await callee.createAnswer());
      await caller.setRemoteDescription(callee.localDescription!);
      ...
      const audioTransport = callee
        .getTransceivers()
        .find((transceiver) => transceiver.kind === "audio")!.dtlsTransport;
      const iceUfrag = audioTransport.iceTransport.connection.remotePassword;

      // Act: subsequent offerでBUNDLE joinするがextmapをharmonizeしない。
      await caller.setLocalDescription(await caller.createOffer());
      const unharmonized = replaceMediaExtmapId(
        caller.localDescription!.sdp,
        "video",
        1,
      );
      await expect(
        callee.setRemoteDescription({ type: "offer", sdp: unharmonized }),
      ).rejects.toThrow(/extmap id .* remapped/);

      // Assert: SRD失敗でICE credentialsは変わらず、signalingはstable。
      expect(callee.signalingState).toBe("stable");
      expect(audioTransport.iceTransport.connection.remotePassword).toBe(
        iceUfrag,
      );
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
  ...
  test("BUNDLE tagにだけa=tls-idを出しnon-tagとgroup外は独立に扱う", async () => {
    // Arrange: max-compat の audio/video 初期offerを作る。
    const caller = new RTCPeerConnection({ iceServers: [] });
    caller.addTransceiver("audio", { direction: "sendonly" });
    caller.addTransceiver("video", { direction: "sendonly" });
    ...
    try {
      const offer = await caller.createOffer();
      const sections = sdpMediaSections(offer.sdp);
      const bundle = offer.sdp.match(/a=group:BUNDLE ([^\r\n]+)/);
      expect(bundle?.[1]?.split(" ").length).toBeGreaterThan(1);
      const tagMid = bundle![1]!.split(" ")[0]!;
      const tagSection = sections.find((section) =>
        section.includes(`a=mid:${tagMid}`),
      );
      const nonTagSections = sections.filter(
        (section) =>
          section.startsWith("m=") && !section.includes(`a=mid:${tagMid}`),
      );

      // Assert: BUNDLE tagだけがtls-idを持ち、memberは重複掲載しない。
      expect(tagSection).toMatch(/a=tls-id:/);
      for (const section of nonTagSections) {
        expect(section).not.toMatch(/a=tls-id:/);
      }

      await caller.setLocalDescription(offer);
      expect(countTlsIdLines(caller.localDescription!.sdp)).toBe(1);
    } finally {
      await caller.close();
    }
  });

  test("BUNDLE tag-switchとgroup外m-lineはtls-id掲載位置を付け替える", async () => {
    // Arrange: audio/video/application の max-compat offer を作る。
    const caller = new RTCPeerConnection({ iceServers: [] });
    caller.addTransceiver("audio", { direction: "sendonly" });
    caller.addTransceiver("video", { direction: "sendonly" });
    caller.createDataChannel("tls-id");

    try {
      const offer = await caller.createOffer();
      const mids = [...offer.sdp.matchAll(/^a=mid:(\S+)/gm)].map(
        (match) => match[1]!,
      );
      expect(mids.length).toBe(3);
      const partial = offer.sdp.replace(
        /a=group:BUNDLE [^\r\n]+/,
        `a=group:BUNDLE ${mids[0]} ${mids[1]}`,
      );
      (caller as unknown as { lastCreatedOffer?: unknown }).lastCreatedOffer =
        undefined;
      await caller.setLocalDescription({ type: "offer", sdp: partial });
      const partialSections = sdpMediaSections(caller.localDescription!.sdp);
      const outside = partialSections.find((section) =>
        section.includes(`a=mid:${mids[2]}`),
      );
      expect(outside).toMatch(/a=tls-id:/);
      expect(
        partialSections.find((section) => section.includes(`a=mid:${mids[0]}`)),
      ).toMatch(/a=tls-id:/);
      expect(
        partialSections.find((section) => section.includes(`a=mid:${mids[1]}`)),
      ).not.toMatch(/a=tls-id:/);

      // Act: tag を入れ替えた subsequent offer をSLDする。
      await caller.close();
    } finally {
      await caller.close().catch(() => undefined);
    }

    const switcher = new RTCPeerConnection({ iceServers: [] });
    switcher.addTransceiver("audio", { direction: "sendonly" });
    switcher.addTransceiver("video", { direction: "sendonly" });
    try {
      const first = await switcher.createOffer();
      await switcher.setLocalDescription(first);
      const bundleMids = first.sdp
        .match(/a=group:BUNDLE ([^\r\n]+)/)![1]!
        .split(" ");
      expect(bundleMids.length).toBe(2);
      const switched = (await switcher.createOffer()).sdp.replace(
        /a=group:BUNDLE [^\r\n]+/,
        `a=group:BUNDLE ${bundleMids[1]} ${bundleMids[0]}`,
      );
      (switcher as unknown as { lastCreatedOffer?: unknown }).lastCreatedOffer =
        undefined;
      await switcher.setLocalDescription({ type: "offer", sdp: switched });
      const sections = sdpMediaSections(switcher.localDescription!.sdp);
      expect(
        sections.find((section) => section.includes(`a=mid:${bundleMids[1]}`)),
      ).toMatch(/a=tls-id:/);
      expect(
        sections.find((section) => section.includes(`a=mid:${bundleMids[0]}`)),
      ).not.toMatch(/a=tls-id:/);
    } finally {
  ...
  test("BUNDLE split後は別RTP sessionの同じextmap IDを拒否しない", async () => {
    // Arrange: bundled の audio/video に MID と TWCC を載せる。
    const headerExtensions = {
      audio: [useSdesMid(), useTransportWideCC()],
      video: [useSdesMid(), useTransportWideCC()],
    };
    ...
    try {
      ...
      // Act: subsequent offerでBUNDLEを外し、video sessionでは id=1 をTWCCだけにする。
      const splitOffer = await caller.createOffer();
      (caller as unknown as { lastCreatedOffer?: unknown }).lastCreatedOffer =
        undefined;
      const splitSdp = sdpMediaSections(stripBundleGroup(splitOffer.sdp))
        .map((section) => {
          ...
          return section
            .replace(
              `a=extmap:1 ${RTP_EXTENSION_URI.sdesMid}`,
              `a=extmap:1 ${RTP_EXTENSION_URI.transportWideCC}`,
            )
            .replace(
              new RegExp(
                `a=extmap:2 ${RTP_EXTENSION_URI.transportWideCC}\\r?\\n`,
              ),
              "",
            );
        })
        .join("");
      await caller.setLocalDescription({ type: "offer", sdp: splitSdp });
      await callee.setRemoteDescription(caller.localDescription!);

      // Assert: split先の新sessionでは同じ数値IDの別URIが合法。
      expect(callee.signalingState).toBe("have-remote-offer");
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
    }
  ...
  test("失敗したsetRemoteDescriptionはtrack/transceiver eventを発火しない", async () => {
    // Arrange: 先頭m-lineは有効、後続videoだけ未知codecにしてSRDを失敗させる。
    const caller = new RTCPeerConnection({ iceServers: [] });
    const callee = new RTCPeerConnection({ iceServers: [] });
    ...
    try {
      await caller.setLocalDescription(await caller.createOffer());
      const invalidOffer = caller.localDescription!.sdp.replace(/VP8/g, "H264");

      // Act: 後続m-lineのcodec交渉を失敗させる。
      await expect(
        callee.setRemoteDescription({ type: "offer", sdp: invalidOffer }),
      ).rejects.toThrow("negotiate codecs failed");

      // Assert: 先行m-lineのeventもrollback不能なので出してはいけない。
  ...
  test("offerにtls-idが無いanswerへa=tls-idを挿入しない", async () => {
    // Arrange: weriftのlocal offerからtls-idを除去してlegacy peerを模倣する。
    const caller = new RTCPeerConnection({ iceServers: [] });
    const callee = new RTCPeerConnection({ iceServers: [] });
    caller.addTransceiver("audio", { direction: "sendonly" });
    caller.addTransceiver("video", { direction: "sendonly" });
    ...
    try {
      await caller.setLocalDescription(await caller.createOffer());
      expect(caller.localDescription!.sdp).toMatch(/a=tls-id:/);
      const offerWithoutTlsId = caller.localDescription!.sdp.replace(
        /^a=tls-id:[^\r\n]+[\r\n]*/gm,
        "",
      );

      // Act: tls-id非対応offerに対するanswerを作る。
      await callee.setRemoteDescription({
        type: "offer",
        sdp: offerWithoutTlsId,
      });
      const answer = await callee.createAnswer();
  ...
  test("answerのextmap directionはremote sendonlyをrecvonlyへ反転する", async () => {
    ...
    try {
      ...
      // Assert: media directionと一致するrecvonlyはqualifierを省略して矛盾しない。
      expect(callee.localDescription!.sdp).toMatch(/^a=recvonly$/m);
  ...
  test("remote answerに残った4096はoffererがusable IDへremapしない", async () => {
    ...
    try {
      ...
      // Assert: offerに無いnegotiation IDはlive remapせずSRD自体を拒否する。
  ...
  test("初回BUNDLE後のnon-tag senderは共有transportのRR/NACK/PLI/TWCCを受ける", async () => {
    // Arrange: max-compatの2本の送信transceiverを初回BUNDLEする。
    const caller = new RTCPeerConnection({ iceServers: [] });
    const callee = new RTCPeerConnection({ iceServers: [] });
    caller.addTransceiver(new MediaStreamTrack({ kind: "audio" }), {
      direction: "sendonly",
    ...
    try {
      ...
      const tagMid = caller
        .localDescription!.sdp.match(/a=group:BUNDLE ([^\r\n]+)/)![1]!
        .split(" ")[0]!;
      const nonTagSender = caller
        .getTransceivers()
        .find((transceiver) => transceiver.mid !== tagMid)!.sender;
      const sharedId = nonTagSender.dtlsTransport.id;
      expect(
        caller
          .getTransceivers()
          .every((transceiver) => transceiver.dtlsTransport.id === sharedId),
      ).toBe(true);

      // Act: 共有transportへnon-tag SSRC宛のfeedbackを流す。
      const received = await collectSenderFeedback(caller, nonTagSender);

      // Assert: 旧独立sessionではなく共有sessionから届く。
      expect(received.rr).toBe(true);
      expect(received.nack).toBe(true);
      expect(received.pli).toBe(true);
      expect(received.twcc).toBe(true);
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
    }
  }, 30_000);

  test("unbundledからBUNDLE joinしたnon-tag senderも共有transportのfeedbackを受ける", async () => {
    // Arrange: まずBUNDLE無しで2本の送信を接続する。
    const caller = new RTCPeerConnection({
      iceServers: [],
      bundlePolicy: "max-compat",
    });
    ...
    try {
      ...
      const firstTransports = new Set(
        caller
          .getTransceivers()
          .map((transceiver) => transceiver.dtlsTransport.id),
      );
      expect(firstTransports.size).toBe(2);

      // Act: subsequent offerでBUNDLEへjoinする。
      await caller.setLocalDescription(await caller.createOffer());
      await callee.setRemoteDescription(caller.localDescription!);
      await callee.setLocalDescription(await callee.createAnswer());
      await caller.setRemoteDescription(callee.localDescription!);
      ...
      const nonTagSender = caller
        .getTransceivers()
        .find((transceiver) => transceiver.mid !== tagMid)!.sender;
      const sharedId = caller.getTransceivers()[0]!.dtlsTransport.id;
      expect(
        caller
          .getTransceivers()
          .every((transceiver) => transceiver.dtlsTransport.id === sharedId),
      ...
      // Assert: join後の共有transportからnon-tag senderへ届く。
      expect(received.rr).toBe(true);
      expect(received.nack).toBe(true);
      expect(received.pli).toBe(true);
      expect(received.twcc).toBe(true);
  ...
  test("queued invalid ICEとreject answerでもlocal senderは止まらない", async () => {
    // Arrange: 初回local offerのあと、remote answer前に不正candidateをqueueする。
    const caller = new RTCPeerConnection({ iceServers: [] });
    const callee = new RTCPeerConnection({ iceServers: [] });
    const audio = caller.addTransceiver(
      new MediaStreamTrack({ kind: "audio" }),
      { direction: "sendonly" },
    );
    const channel = caller.createDataChannel("queued-ice");
    callee.addTransceiver("audio", { direction: "recvonly" });
    ...
    try {
      ...
      await callee.setRemoteDescription(caller.localDescription!);
      const answer = await callee.createAnswer();
      const rejectedAudio = answer.sdp.replace(/^m=audio \d+/m, "m=audio 0");

      // Act: reject m-lineを含むanswerを、queue済み不正candidateと一緒に適用する。
      await expect(
        caller.setRemoteDescription({ type: "answer", sdp: rejectedAudio }),
      ).rejects.toThrow();

      // Assert: SRD失敗後もlocal sender/DataChannelは破棄されない。
      expect(audio.stopped).toBe(false);
      expect(audio.sender.stopped).toBe(false);
      expect(channel.readyState).not.toBe("closed");
      expect(caller.signalingState).toBe("have-local-offer");
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
    }
  ...
  test("connecting中のremote answer fingerprint変更もNotSupportedErrorにする", async () => {
    // Arrange: 初回answer適用でDTLS startをhandshake前に止める。
    const caller = new RTCPeerConnection({ iceServers: [] });
    const callee = new RTCPeerConnection({ iceServers: [] });
    caller.createDataChannel("connecting-dtls");
    ...
    try {
      ...
      await callee.setLocalDescription(await callee.createAnswer());
      const dtls = caller.dtlsTransports[0]!;
      const originalStart = dtls.start.bind(dtls);
      let startEntered!: () => void;
      const startEnteredPromise = new Promise<void>((resolve) => {
        startEntered = resolve;
      });
      ...
      await caller.setRemoteDescription(callee.localDescription!);
      await startEnteredPromise;
      expect(dtls.state).not.toBe("connected");

      await caller.setLocalDescription(await caller.createOffer());
      await callee.setRemoteDescription(caller.localDescription!);
      const answer = await callee.createAnswer();

      // Act: まだconnectingの既存associationへ新しいfingerprintのanswerを渡す。
      await expect(
        caller.setRemoteDescription({
          type: "answer",
          sdp: tamperFingerprints(answer.sdp),
      ...
      // Assert: 未実装のreplacementを拒否し、現行associationはfailedにしない。
      expect(dtls.state).not.toBe("failed");
      expect(caller.signalingState).toBe("have-local-offer");
    } finally {
      releaseStart?.();
      await Promise.allSettled([caller.close(), callee.close()]);
  ...
  test("初回BUNDLEのpending-to-pending extmap衝突はSRDでrejectする", async () => {
    // Arrange: live mapが空のまま、同じBUNDLE sessionへ衝突するextmapを載せる。
    const headerExtensions = {
      audio: [useSdesMid()],
      video: [useTransportWideCC()],
    };
    ...
    try {
      ...
      // Act: 同一idの別URIを同じtargetへ同時に載せるofferをSRDする。
      const colliding = replaceMediaExtmapId(
        caller.localDescription!.sdp,
        "video",
        1,
      );
      await expect(
        callee.setRemoteDescription({ type: "offer", sdp: colliding }),
      ).rejects.toThrow(/extmap id .* remapped/);
  ...
  test("初回negotiationのanswerがofferのextmap IDをremapするとSRDをrejectする", async () => {
    // Arrange: local offerはMIDを通常IDで提示する。
    const headerExtensions = { video: [useSdesMid()] };
    const caller = new RTCPeerConnection({ iceServers: [], headerExtensions });
    const callee = new RTCPeerConnection({ iceServers: [], headerExtensions });
    caller.addTransceiver("video", { direction: "sendonly" });
    ...
    try {
      ...
      await callee.setRemoteDescription(caller.localDescription!);
      const answer = await callee.createAnswer();
      const remapped = answer.sdp.replace(
        /a=extmap:(\d+) /,
        (_match, id: string) => `a=extmap:${Number(id) + 1} `,
      );

      // Act: offerと違うvalid-range IDのanswerをSRDする。
      await expect(
        caller.setRemoteDescription({ type: "answer", sdp: remapped }),
      ).rejects.toThrow(/answer remapped extmap/);

      // Assert: 初回negotiationでもlive mapが空のままremapを通さない。
      expect(caller.signalingState).toBe("have-local-offer");
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
    }
  ...
  test("unsupported extmapとのBUNDLE ID衝突はfilter前にrejectする", async () => {
    // Arrange: calleeはvideo MIDだけsupportし、audio URIは未対応。
    const caller = new RTCPeerConnection({
      iceServers: [],
      bundlePolicy: "max-bundle",
      headerExtensions: {
    ...
    try {
      await caller.setLocalDescription(await caller.createOffer());
      const colliding = rewriteMediaExtmap(
        rewriteMediaExtmap(
          caller.localDescription!.sdp,
          "audio",
          1,
          "urn:example:unsupported-A",
        ),
        "video",
        1,
      ...
      // Act: unsupported URIとMIDが同じIDを共有するBUNDLE offerをSRDする。
      await expect(
        callee.setRemoteDescription({ type: "offer", sdp: colliding }),
      ).rejects.toThrow(/extmap id .* remapped/);

      // Assert: capability filter後のvalidatorだけに頼らず拒否する。
      expect(callee.signalingState).toBe("stable");
      expect(callee.getTransceivers()).toHaveLength(0);
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
  ...
  test("recvonly m-lineのsendonly extmapはSRDでrejectする", async () => {
    // Arrange: audio MIDを付けたofferを作り、RFC 8285の矛盾directionへ改変する。
    ...
    try {
      await caller.setLocalDescription(await caller.createOffer());
      const invalid = sdpMediaSections(caller.localDescription!.sdp)
        .map((section) => {
          if (!section.startsWith("m=audio")) return section;
          return section
            .replace(/^a=sendonly$/m, "a=recvonly")
            .replace(/a=extmap:(\d+)/, "a=extmap:$1/sendonly");
        })
        .join("");

      // Act: recvonly media + sendonly extmap のofferをSRDする。
      await expect(
        callee.setRemoteDescription({ type: "offer", sdp: invalid }),
      ).rejects.toThrow(/contradicts media direction/);

      // Assert: 明示的なerror組み合わせはsignalingを進めない。
      expect(callee.signalingState).toBe("stable");
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
    }
  ...
  test("createAnswerはsendrecv extmapをanswerのm-line方向へ絞り込む", async () => {
    // Arrange: sendrecv offerに明示sendrecv extmapを載せる。
    const headerExtensions = { audio: [useSdesMid()] };
    const caller = new RTCPeerConnection({ iceServers: [], headerExtensions });
    const callee = new RTCPeerConnection({ iceServers: [], headerExtensions });
    ...
    try {
      await caller.setLocalDescription(await caller.createOffer());
      const offer = sdpMediaSections(caller.localDescription!.sdp)
        .map((section) =>
          section.startsWith("m=audio")
            ? section.replace(/a=extmap:(\d+)/, "a=extmap:$1/sendrecv")
            : section,
        )
        .join("");
      await callee.setRemoteDescription({ type: "offer", sdp: offer });

      // Act: 新規remote transceiverのrecvonlyでanswerする。
      const answer = await callee.createAnswer();
      await callee.setLocalDescription(answer);

      // Assert: recvonly m-lineにsendrecv qualifierを残さない。
  ...
  test("inactive m-lineのsendrecvと省略extmapはSRDできる", async () => {
    // Arrange: inactive audioに明示sendrecvとqualifier省略のextmapを載せる。
    const headerExtensions = {
      audio: [useSdesMid(), useTransportWideCC()],
    };
    const caller = new RTCPeerConnection({ iceServers: [], headerExtensions });
    const callee = new RTCPeerConnection({ iceServers: [], headerExtensions });
    caller.addTransceiver("audio", { direction: "inactive" });

    try {
      await caller.setLocalDescription(await caller.createOffer());
      const inactive = sdpMediaSections(caller.localDescription!.sdp)
        .map((section) => {
          if (!section.startsWith("m=audio")) return section;
          return section.replace(
            new RegExp(`a=extmap:(\\d+) ${RTP_EXTENSION_URI.sdesMid}`),
            `a=extmap:$1/sendrecv ${RTP_EXTENSION_URI.sdesMid}`,
          );
        })
        .join("");

      // Act: RFC 8285のinactive継承規則に合うofferをSRDする。
      await callee.setRemoteDescription({ type: "offer", sdp: inactive });

      // Assert: sendrecv qualifierも省略もinactive streamではerrorにしない。
      expect(callee.signalingState).toBe("have-remote-offer");
      expect(callee.getTransceivers()).toHaveLength(1);
    } finally {
      await Promise.allSettled([caller.close(), callee.close()]);
...
function tamperFingerprintForKind(sdp: string, kind: "audio" | "video") {
  return sdp
    .split(/(?=^m=)/m)
    .map((section) =>
      section.startsWith(`m=${kind}`) ? tamperFingerprints(section) : section,
    )
    .join("");
}

function stripBundleGroup(sdp: string) {
  return sdp
    .split(/\r\n|\n/)
    .filter((line) => !line.startsWith("a=group:BUNDLE"))
    .join("\r\n");
...
function sdpMediaSections(sdp: string) {
  return sdp.split(/(?=^m=)/m);
}

function replaceMediaExtmapId(sdp: string, kind: string, id: number) {
  return sdpMediaSections(sdp)
    .map((section) =>
      section.startsWith(`m=${kind}`)
        ? section.replace(/a=extmap:\d+/, `a=extmap:${id}`)
        : section,
    )
    .join("");
}

function rewriteMediaExtmap(
  sdp: string,
  kind: string,
  id: number,
  uri: string,
  ...
  return sdpMediaSections(sdp)
    .map((section) =>
      section.startsWith(`m=${kind}`)
        ? section.replace(/a=extmap:\d+(?:\/\S+)? \S+/, `a=extmap:${id} ${uri}`)
        : section,
    )
    .join("");
}

function extmapsInMedia(sdp: string, kind: string) {
  const section =
    sdpMediaSections(sdp).find((media) => media.startsWith(`m=${kind}`)) ?? "";
  return [...section.matchAll(/a=extmap:(\d+)(?:\/\S+)? (\S+)/g)].map(
    (match) => ({
      id: Number(match[1]),
      uri: match[2]!,
    }),
...
function countTlsIdLines(sdp: string) {
  return [...sdp.matchAll(/^a=tls-id:/gm)].length;
}

function getPeerRouter(pc: RTCPeerConnection) {
  return (pc as unknown as { router: RtpRouter }).router;
}

async function collectSenderFeedback(
  pc: RTCPeerConnection,
  sender: RTCRtpSender,
) {
  const received = { rr: false, nack: false, pli: false, twcc: false };
  sender.onRtcp.subscribe((packet) => {
    if (packet.type === RtcpRrPacket.type) received.rr = true;
    if (packet.type === RtcpTransportLayerFeedback.type) {
      const feedback = (packet as RtcpTransportLayerFeedback).feedback;
      if (feedback.count === TransportWideCC.count) received.twcc = true;
    }
  });
  sender.onGenericNack.subscribe(() => {
    received.nack = true;
  });
  sender.onPictureLossIndication.subscribe(() => {
    received.pli = true;
  });
  const router = getPeerRouter(pc);
  const transportId = sender.dtlsTransport.id;
  router.routeRtcp(
    new RtcpRrPacket({
      ssrc: 1,
      reports: [
