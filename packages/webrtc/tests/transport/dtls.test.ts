...
describe("RTCDtlsTransportTest", () => {
  ...
  test("client 開始直後の stop は closed を維持し failed へ遷移しない", async () => {
    // Arrange: 未開始の client/server pair を用意する。
    const [server, client] = await createUnstartedDtlsSessions();
    const target = client.role === "client" ? client : server;
    const peer = target === client ? server : client;
    const startPromise = target.start();
    void startPromise.catch(() => undefined);
    const peerStart = peer.start();
    void peerStart.catch(() => undefined);

    try {
      // Act: client の 100ms 遅延 connect 前に停止する。
      await setTimeout(10);
      await target.stop();
      await setTimeout(250);

      // Assert: 遅延した connect 失敗で closed が failed へ上書きされない。
      expect(target.state).toBe("closed");
    } finally {
      await Promise.allSettled([startPromise, peerStart, peer.stop()]);
    }
  });

  test("DTLS close 後は SRTP/SRTCP 送信許可が残らず統計も増えない", async () => {
    // Arrange: SRTP profile 付きで接続済みの pair を用意する。
    const profile = ProtectionProfileAes128CmHmacSha1_80;
    const [client, server] = await dtlsTransportPair({
      protocolVersions: [DtlsVersion.V1_3],
      srtpProfiles: [profile],
    } as unknown as ConstructorParameters<typeof RTCDtlsTransport>[0]);
    const before = (await client.getStats()).find(
      (s): s is RTCTransportStats => s.type === "transport",
    );

    try {
      // Act: close 通知後に RTP/RTCP 送信を試みる。
      await client.stop();
      const rtpBytes = await client.sendRtp(
        Buffer.from("after-close"),
        new RtpHeader({ ssrc: 1, payloadType: 96 }),
      );
      const rtcpBytes = await client.sendRtcp([
        new RtcpRrPacket({ ssrc: 1, reports: [] }),
      ]);
      const after = (await client.getStats()).find(
        (s): s is RTCTransportStats => s.type === "transport",
      );

      // Assert: 送信は許可されず、統計も増加しない。
      expect(rtpBytes).toBe(0);
      expect(rtcpBytes).toBe(0);
      expect(client.state).toBe("closed");
      expect((after?.packetsSent ?? 0) - (before?.packetsSent ?? 0)).toBe(0);
    } finally {
      await Promise.allSettled([client.stop(), server.stop()]);
    }
  });
