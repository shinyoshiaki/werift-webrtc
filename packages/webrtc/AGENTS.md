...
* Optional `werift/polyfill` (`src/polyfill`) installs browser globals and `getUserMedia` via `mediaRegister`; it is not re-exported from `src/index.ts`. On Node it also complements a missing or `Node.js/<major>` `navigator.userAgent` with a Chromium-compatible value. Existing non-Node User-Agents are kept unless `userAgent` is passed.
