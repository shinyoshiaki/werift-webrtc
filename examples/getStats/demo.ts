...
async function demonstrateGetStats() {
  ...
  try {
    ...
    const pcStat = Array.from(dcStats.values()).find(s => s.type === "peer-connection") as any;
    if (pcStat) {
    ...
    const mediaSourceStats = Array.from(mediaStats.values()).filter(s => s.type === "media-source");
    ...
    for (const [id, stat] of detailedStats) {
      ...
      if (stat.type === "peer-connection") {
        ...
        console.log(`     dataChannelsOpened: ${pcStat.dataChannelsOpened || 0}`);
        console.log(`     dataChannelsClosed: ${pcStat.dataChannelsClosed || 0}`);
    ...
    console.log(`   Time difference: ${(timestamp2 - timestamp1).toFixed(2)}ms`);
    console.log();
