関連するWPT周りの構成と実行経路を先に洗い出して、タスクを実装可能な粒度まで落とし込みます。



  │ "**/AGENTS.md"

  │ "\bwpt\b|WPT|web-platform-tests|coverage|allowlist"

  │ packages/webrtc/package.json


  │ "\bwpt(:list|:coverage)?\b|allowlist|coverage|web-platform-tests" in
  │ **/*.{ts,js,json,md} (packages/webrtc)
  ...
  │ "function printTargetList|printTargetList|formatProgressEvent|writeMarkdownR
  │ eport|findRegressions|summarize"
  │ (packages/webrtc/tools/wpt-runner/runner.ts)

  │ packages/webrtc/wpt/baseline.json

  │ packages/webrtc/wpt/coverage-baseline.json

  │ "peerConnectionApiCompatibility|WPT|upstream|compatibility" in **/*.{ts,md}
  │ (packages/webrtc/src, packages/webrtc/tests)

WPTレポート生成と、実装側で改善対象になりやすい互換性メモを確認して、詳細化の前提を固めます。
  ...
  │ coverage/webrtc-wpt/results.json

  │ coverage/webrtc-wpt/results.md

  │ cd /workspace && npm run wpt:list --workspace packages/webrtc | sed -n
  │ '1,80p'
  ...
  │ "createDataChannel\(|getConfiguration\(|bundlePolicy|iceCandidatePoolSize|ma
  │ xPacketLifeTime|maxRetransmits|canTrickleIceCandidates|removeTrack\(|ontrack
  │ |getStats\(" in **/*.ts (packages/webrtc/src)

  │ "packages/webrtc/tests/**/*Compatibility*.ts"

  │ "packages/webrtc/tests/wpt/**/*.ts"

互換性改善の候補を特定するため、RTCPeerConnection/RTCDataChannel 周りのWPT寄りテストと実装の接点を追加で確認します。

  │ "class RTCPeerConnection|createDataChannel\(|removeTrack\(|getConfiguration\
  │ (|setLocalDescription\(|setRemoteDescription\(|addIceCandidate\(|canTrickleI
  │ ceCandidates|bundlePolicy|ontrack" (packages/webrtc/src/peerConnection.ts)

  │ "createDataChannel coerces|constructor applies WebIDL-style
  │ validation|removeTrack is a no-op|ontrack lives on the
  │ prototype|canTrickleIceCandidates|setLocalDescription\(rollback\)|setRemoteD
  │ escription\(rollback\)|addIceCandidate\(null\)" in **/*.ts
  │ (packages/webrtc/tests)

  │ "class RTCDataChannel|maxPacketLifeTime|maxRetransmits|ordered|protocol|labe
  │ l|bufferedAmount|close\(|send\(" (packages/webrtc/src/dataChannel.ts)

  │ "dummy|MediaStreamTrack|getUserMedia|track-stats|getStats|stats" in
  │ **/*.{ts,md} (packages/webrtc/src, packages/webrtc/tests)
  ...
  │ "getStats\(|RTCStats|statsId|messagesSent|bytesSent|messagesReceived|bytesRe
  │ ceived" in **/*.ts (packages/webrtc/src)

  │ "createDataChannel\(|coerceUnsignedShort|TypeError|DOMException|bufferedAmou
  │ ntLowThreshold|maxPacketLifeTime|maxRetransmits|negotiated|id\s*>\s*65534"
  │ in **/*.ts (packages/webrtc/src/sctpManager.ts, packages/webrtc/src)

  │ "track-stats|createDataChannel|bundlePolicy|RTCDataChannel-send|RTCDataChann
  │ el-close|getStats" in **/*.{json,md,ts} (packages/webrtc/wpt/allowlist.json,
  │ packages/webrtc/README.md, packages/webrtc/tests)
...
現状の成功ファイル一覧を実データで把握するため、WPT を一度走らせて生成レポートを確認します。

● Run WebRTC WPT suite (shell)
  │ cd /workspace && npm run wpt --workspace packages/webrtc

  │ coverage/webrtc-wpt/results.md

  │ coverage/webrtc-wpt/results.json

● Summarize passing WPT files (shell)
  │ cd /workspace && node - <<'NODE'
  │ const fs = require('fs');
  │ const path = '/workspace/coverage/webrtc-wpt/results.json';
  │ const report = JSON.parse(fs.readFileSync(path, 'utf8'));
  │ const byFile = new Map();…

優先度の高い改善候補を絞るため、部分成功している代表ファイルの失敗内容を抜き出して傾向を見ます。

● Extract representative WPT failures (shell)
  │ cd /workspace && node - <<'NODE'
  │ const fs = require('fs');
  │ const report =
  │ JSON.parse(fs.readFileSync('coverage/webrtc-wpt/results.json','utf8'));
  │ const targets = […

✗ Memory stored wpt tooling
    を出力し、Markd…

**調査結果:** このタスクは既存の WPT ランナー基盤をそのまま使って進められます。現状は **214 target / 1061 subtest 中 283 PASS**、**1件以上 PASS がある file+variant は 70 件**で、一覧は `npm run wpt --workspace packages/webrtc` 実行後に **`coverage/webrtc-wpt/results.md`** へ自動出力されます。
...
- WPT 作業前に **`git submodule update --init --recursive`** が前提
