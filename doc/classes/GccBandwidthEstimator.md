...
[`ProbePacingController`](../interfaces/ProbePacingController.md).[`reserveOutgoingProbe`](../interfaces/ProbePacingController.md#reserveoutgoingprobe)
...
with the previous min/max. `min = 0` floors at [kMinBitrateBps](../variables/kMinBitrateBps.md);
non-finite / non-positive max clears the app cap (target 1 Gbps,
probe 5 Mbps). `start = 0` does not overwrite current_target.

Normalization (pin order): min floored at [kMinBitrateBps](../variables/kMinBitrateBps.md) →
...
> **updatePacketsLost**(`packetsLost`, `numberOfPackets`, `atTimeMs`?): `void`
...
[kStartPhaseLossReportMinPackets](../variables/kStartPhaseLossReportMinPackets.md) packets accumulate.

Not on the thin [BandwidthEstimator](../interfaces/BandwidthEstimator.md) interface — RTCP RR / XR
wiring is GCC-specific (same pattern as [setRoundTripTime](GccBandwidthEstimator.md#setroundtriptime)).
